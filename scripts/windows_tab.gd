...
const categories: Dictionary = {"cpu": "CPU", "network": "Network", "gpu": "GPU", 
"research": "Research", "hacking": "Hacking", "coding": "Coding", "utility": "Utilities"}
...
func get_category_visibility(category: String) -> bool:
    ...
    if Globals.money_level < Data.windows[window].level: return false
    if !Data.windows[window].upgrade.is_empty() and Globals.upgrades[Data.windows[window].upgrade] <= 0: return false
