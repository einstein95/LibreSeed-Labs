...
var premiums_claimed: Dictionary
var daily_premiums_claimed: Dictionary
var premiums: Array
...
func init_vars() -> void :
    ...
    for i: String in Data.premiums:
        if !premiums_claimed.has(i):
            premiums_claimed[i] = false
        if !daily_premiums_claimed.has(i):
            daily_premiums_claimed[i] = false

    if last_date != Time.get_date_dict_from_system():
        for i: String in Globals.daily_premiums_claimed:
            daily_premiums_claimed[i] = false
...
func init_upgrades() -> void :
    ...
    for i: String in research:
        if !Data.research.has(i): continue
...
func update_premiums() -> void :
    if Premiums.features.has("permanent_boost"):
        for i: String in boosts:
            boosts[i].time = 1e+69
...
func claim_premium(premium: String) -> void :
    premiums_claimed[premium] = true
    var reward: float = Data.premiums[premium].reward
    claimabled_tokens += reward
    stats.max_tokens += reward


func claim_daily_premium(premium: String) -> void :
    daily_premiums_claimed[premium] = true
    var reward: float = Data.premiums[premium].daily_reward
    claimabled_tokens += reward
    stats.max_tokens += reward
...
func clear() -> void :
    ...
    premiums_claimed.clear()
    daily_premiums_claimed.clear()
    premiums.clear()
...
func save() -> Dictionary:
    ...
    return {
        ...
        "last_date": Time.get_date_dict_from_system(), 
        "premiums": premiums, 
        "premiums_claimed": premiums_claimed, 
        "daily_premiums_claimed": daily_premiums_claimed, 
        "save_ver": 0
    }
