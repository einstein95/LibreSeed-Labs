...
const progress_buttons: Dictionary = {
	"Upgrades": Utils.menus.UPGRADES,
	"Storage": Utils.menus.STORAGE,
	"Tokens": Utils.menus.TOKENS,
	"Portal": Utils.menus.PORTAL
}
const extras_buttons: Dictionary = {
	"Guide": Utils.menus.GUIDE,
	"Achievements": Utils.menus.ACHIEVEMENTS,
	"Settings": Utils.menus.SETTINGS
}
const windows_buttons: Dictionary = {
	"CPU": "cpu",
	"Network": "network",
	"GPU": "gpu",
	"Research": "research",
...
func _input(event: InputEvent) -> void:
	if event is InputEventKey and event.is_released() and event.keycode == KEY_F10:
		visible = !visible


func set_menu(menu: int, tab: int = 0, pressed: bool = false) -> void:
	if cur_menu == menu and menu == Utils.menu_types.NONE:
		return
	match menu:
...
func update_buttons() -> void:
	...
	var is_transitioning = 1 if transitioning else 0
	$Main/MainContainer/Overlay/ScreenButtons/Container/Desktop.button_mask = is_transitioning
	$Main/MainContainer/Overlay/ScreenButtons/Container/Research.button_mask = is_transitioning
...
func check_new_windows() -> void:
	...
	if new.size() > 0:
		...
		for i: String in new:
			if categories.has(Data.windows[i].category):
				continue
...
func get_new_windows() -> Array[String]:
	...
	for i: String in Data.windows:
		if available_windows.has(i):
			continue

		if !Data.windows[i].requirement.is_empty() and !Globals.unlocks[Data.windows[i].requirement]:
			continue
