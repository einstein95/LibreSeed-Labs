...
func update_all() -> void :
    var time: float = Globals.boosts[name].time
    owned = Premiums.features.has("permanent_boost")
...
func get_visibility() -> bool:
    if !Data.boosts[name].requirement.is_empty() and !Globals.unlocks[Data.boosts[name].requirement]: return false
    if Data.boosts[name].level > Globals.money_level: return false

    return true


func can_purchase() -> bool:
    if Data.boosts[name].cost > Globals.currencies["token"]: return false
