...
func can_purchase() -> bool:
    if Data.upgrades[name].limit > 0 and Globals.upgrades[name] >= Data.upgrades[name].limit: return false
    if Attributes.get_attribute("optimization_points") < 1: return false
