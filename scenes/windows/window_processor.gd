...
func _ready() -> void :
    ...
    count = max(Attributes.get_window_attribute(window, "minimum_cores") - 1, count)
...
func process(delta: float) -> void :
    ...
    for i: ResourceContainer in clocks.get_children():
        i.count = speed * (1.0 + boost.count) * overclock_bonus * Attributes.get_attribute("clock_multiplier")
    main_clock.count *= Attributes.get_window_attribute("processor", "main_core_multiplier")
