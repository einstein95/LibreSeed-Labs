...
func process(delta: float) -> void :
    if floorf(file.count) >= 1:
        ...
        if progress >= goal:
            ...
            output.add(count)
            Globals.stats.quarantined += count
            progress = fmod(progress, goal)

            Signals.quarantined.emit(file, output, count)
            if is_processing():
...
func _on_file_resource_set() -> void :
    progress = 0
    output.set_resource(file.resource, file.variation & ~ Utils.file_variations.INFECTED)
    update_type()
