...
@onready var core: = $PanelContainer / MainContainer / Core
...
func process(delta: float) -> void :
    if power.count >= power.required:
        if transistor.count >= transistor.required and pcb.count >= pcb.required:
            progress += speed * delta
            if progress >= goal:
                var count: float = min(progress / goal, transistor.count / transistor.required, pcb.count / pcb.required)
                core.add(count)
                Globals.stats.gpu_assembled += count
                progress = fmod(progress, goal)
                pcb.pop(count * pcb.required)
                transistor.pop(count * transistor.required)
                audio_player.play()
                if is_processing():
                    core.animate_icon_in_pop(count)
        else:
            progress = 0
        core.production = min(speed / goal, transistor.production / transistor.required, pcb.production / pcb.required)
    else:
        core.production = 0
