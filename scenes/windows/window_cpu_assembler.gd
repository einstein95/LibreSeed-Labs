...
func process(delta: float) -> void :
    if power.count >= power.required:
        if transistor.count >= transistor.required:
            progress += speed * delta
            if progress >= goal:
                var count: float = min(progress / goal, transistor.count / transistor.required)
                core.add(count)
                Globals.stats.cores_assembled += count
            ...
            progress = 0
        core.production = min(speed / goal, transistor.production / transistor.required)
    else:
        core.production = 0
