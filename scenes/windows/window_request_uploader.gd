...
func update_request() -> void:
    ...
    if Data.files.has(file.resource):
        for i: String in Data.requests:
            if !is_request_unlocked(i):
                continue

            if Data.requests[i].file != file.resource:
                continue

            if !Data.requests[i].variation.is_empty() and !file.variation & Data.requests[i].variation.bin_to_int():
                continue

            request = i
            if Globals.requests[request] == 0:
                break
...
func is_request_unlocked(r: String) -> bool:
    if Data.requests[r].requirement.is_empty():
        return true
