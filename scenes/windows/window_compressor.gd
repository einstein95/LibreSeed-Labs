...
func process(delta: float) -> void :
    if floorf(file.count) >= 1:
        ...
        if progress >= goal:
            ...
            Signals.compressed.emit(file, output, count)
...
func update_type() -> void :
    if Data.files.has(file.resource):
        ...
        if file.variation & Utils.file_variations.COMPRESSED:
            goal *= 4
...
func _on_file_resource_set() -> void :
    ...
    if max_level >= 2 and file.variation & Utils.file_variations.COMPRESSED2:
        new_var |= Utils.file_variations.COMPRESSED3
    elif max_level >= 1 and file.variation & Utils.file_variations.COMPRESSED:
        new_var |= Utils.file_variations.COMPRESSED2
    else:
        new_var |= Utils.file_variations.COMPRESSED

    if file.variation & Utils.file_variations.ENHANCED3:
        new_var &= ~ Utils.file_variations.ENHANCED3
    elif file.variation & Utils.file_variations.ENHANCED2:
        new_var &= ~ Utils.file_variations.ENHANCED2
    elif file.variation & Utils.file_variations.ENHANCED:
